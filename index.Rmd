...
To attach ggplot2 run:

```{r, highlight=TRUE}
library(ggplot2)
```

We will also use the dplyr package to manipulate the data:

```{r, highlight=TRUE}
library(dplyr)
```

And the magrittr pipe to make the code easier to read:
...
ggplot(data = mpg, aes(x = displ, y = hwy)) +
  geom_point() +
  geom_point(aes(x = displ, y = cty), colour = "blue")
...
Use `scale_` functions to set visual properties of the aesthetics, such as the colours or shapes used.

```{r, fig.height=3, fig.retina=2}
# Use scale_functions to set visual properties of the aesthetics
ggplot(filter(mpg, manufacturer %in% c('audi', 'volkswagen')),
       aes(x = displ, y = hwy, colour = manufacturer, shape = manufacturer)) +
  geom_point(size = 3) +
  scale_colour_manual(values = c("audi" = "#28b7b7", "volkswagen" = "violet")) +
...
### 2 
- Map a continuous variable to `colour`, `size` and `shape`. How do these aesthetics behave differently\n for categorical vs. continuous variables? Hint: use `glimpse(mpg)` to identify variable types.
...
geom_smooth() takes data points and returns a regression model with confidence intervals. For example, if we just replace `geom_point()` with `geom_smooth()`, we get a loess curve.  
