...
You can also specify things manually. Hint use `colours()` to get the list of built in colours. Use hex values for any colour `'#28b7b7'`. 
...
As an example, the stat `ggplot2::stat_summary()` is the statistical transformation that provides a statistical summary of the chosen variable, including mean/median/mode, max/min, quartiles. The default geom for this stat is the `ggplot2::geom_pointrange()` - which shows the median value as a point, and lines to indicate the range of values recorded by the variable.
...
## Bar charts
Some geoms plot raw data (like `ggplot2::geom_point()`) others perform transformations in the background. We've already seen this with `ggplot2::geom_smooth()`. Look at what happens when we plot a bar chat of vehicle class.
...
Rather than returning every observation, ggplot has summarized the data using the count statistic. This is because this is the default behavior for a bar chart - `ggplot2::stat_count()` is the default stat for `ggplot2::geom_bar()`. The same chart can therefore be produced using `ggplot2::ggplot(data = mpg, aes(x=class)) + ggplot2::stat_count()`. 
...
has its own grammar for constructing charts, with power and flexibility rivalling that of ggplot2. 
...
- Hint: run `?ggplot2::stat_smooth()` to open the documentation for `ggplot2::stat_smooth()`, see how the transformation can be controlled.
