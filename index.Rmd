...
- Practise creating plots yourself
- Signpost resources
...
Extension: Try removing the confidence interval from the smooth line. Also try changing the linetype to dashed or dotted.

##Stats and Geoms

Both geoms and stats can be used to make plots in the R package ggplot2, and they will give corresponding results. The difference between geoms and stats can be summarised as follows:
...
- stats stand for 'statistical transformations' - these objects summarise the data in different ways, to map the data to the chosen geom in the chosen way
...
For most of the time you will want to use the default stat for a chosen geom (or vice versa), but sometimes we will want to specify this explicitly, as in the next example.
...
Rather than returning every observation, ggplot has summarised the data using the count statistic. This is because this is the default behaviour for a bar chart - `stat_count` is the default stat for `geom_bar`. The same chart can therefore be produced using `ggplot(data=mpg, aes(x=class)) + stat_count()`. 
...
2. Now specify a y co-ordinate mapping to show the average fuel displacement of each class of car. 
*Hint* you will need to also specify a correct 'stat' and function to use.
...
4. Manually change the colours of the fill being used. Also change the font size and angle of the x axis text labels so that these can be read in the viewer.
...
1. Create a basic scatter plot showing highway fuel efficiency vs city fuel efficiency. Add a google docs theme from the ggthemes package; and colour the points based on the number of cylinders the car has.
*Hint* you will need to use `as.factor(cyl)` rather than just `cyl` when mapping the colour.
...
4. Try changing the x and y axis so that the scale starts at zero. 

5. Change the title of the legend to 'Number of Cylinders' and re-name the labels with the text version of the number (e.g. 'four' instead of '4')
