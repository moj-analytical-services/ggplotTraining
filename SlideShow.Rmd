...
- It covers all the basic useage of R and is pretty up to date. 
...
knitr::include_graphics("grammerofgraphics.png")
```
...
`hwy` contains the cars fuel effciency on the motorway, low scores = more fuel consumption
...
- how that data should be mapped to the coordinate system, defined using `aes()` (short for aestetics). Here we use x and y axis. 
- a geometric to draw the aestetics with
...
3. Make a scatterplot of `hwy` vs. `cty`
4. Make a scatterplot of `class` vs. `drv`. Why isn't this plot useful?

## Aesthetics

There are plenty of ways to map aethetics in ggplot. 

For example, we could set the colour of the point to be determined by the vehcile class. 
...
ggplot does some clever things when deciding what colours to use - for factoral variables it will assign each factor a unique colour, whilst for coninuous variables it will assign a colour scale. These can be customised (we'll cover this later). 
...
You don't *have* to map aestetics onto variables, you can specify them manually. For example, you can define the colour of the points:
...
- Map a continous variable to `color`, `size` and `shape`. How do these aesthetics behave differently for categorical vs. continous variables? Hint: use `glimpse(mpg)` to identify variable types. 
...
There are two ways of doing this; `facet_wrap()` or `facet_grid()`. The main argument here is an R 'formula', where the faceted variable is preceeded by a tilde ~ (e.g. ~class). To facet multiple variables, they can be separated by a tilde (e.g. drv ~ cyl)
...
We can add the points as well, using the + operator. Note that the order of layers determines their order on the plot. As points are after the smooth line, they will be draw ontop (and obscure) the line. 
...
## Bar charts 
Some geoms plot raw data (like `geom_point()`) others perform transformations in the background. We've already seen this with `geom_smooth()`. Look at what happens when we plot a bar chat of vehcle class.
...
You can control ggplot transformation more precicely by issuing the `stat_` command. Heres a few examples: Mean, min and max values in one plot.
```{r, fig.height=4}
...
Bootstraped 95% confidence intervals:
```{r}
ggplot(data=mpg, aes( x=class, y=hwy))+
...
Because there are multiple observations, some are **overplotted**. To correct this, you can add some random noise to the data with `position="jitter"` or `geom_jitter`. This is a bit of a compromise - you either have a chart that is accurate but suffers from overplotting, or one that contains some random noise but reveals the size of the data. 
...
plot2<-
  ggplot(data=mpg, aes(x=cty, y=hwy))+
  geom_jitter(color='red')+
  labs(title="Highway fuel effeciency\nby number of cylinders", 
       subtitle="Note, the points are jittered", x="Number of cylinders", y="Fuel effeciency")
...
There are tons of packages that contain premade themes for ggplot: see http://www.ggplot2-exts.org/gallery/ for a few
...
note toe self: add extension themes tomorrow, how to do custom themes, also ggrepel labels. 
